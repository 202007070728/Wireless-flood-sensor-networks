...
def update_vertex_in_idf(ida_file_path, coordinates):
    ...
    for line in lines:
        ...
        updated_lines.append(f'{wall_index} 4 {coord_str} {rest}\n')
        print("已更新")
...
def Get_wall_vertices_world_coordinates(ifc_file_path,ida_file_path):
    """
    Load an IFC file and extract world coordinates of all wall vertices.

    :param ifc_file_path: Path to the IFC file
    :return: A list of tuples, where each tuple contains the wall index and its vertices in world coordinates
    """
    # Load the IFC file
    ...
    for line in lines:
        ...
        if 'END_WALLS' in line:
            ...
            updated_lines.append(line)
            continue
    # Select all wall entities
    walls = ifc_file.by_type('IfcWall')

    # Prepare a list to hold all the vertices in world coordinates
    world_vertices = []
    points_by_wall = {}
    # Iterate through each wall to get its absolute coordinates
    for i, wall in enumerate(walls):
        ...
        for j in range(0, len(verts), 3):
            ...
            wall_vertices.append(np.round(transformed_vert[:3], 1).tolist())

        # Now wall_vertices contains all the vertices in world coordinates for this wall
        points_by_wall[i + 1] = wall_vertices

    # Print the points for each wall
    for wall_index, vertices in points_by_wall.items():
        print(f"墙{wall_index}的顶点坐标：")
        # for vertex in vertices:
        # print(vertex)
        print(points_by_wall[wall_index])  # Add an empty line for better readability between walls
        for axis in range(3):  # 0: x, 1: y, 2: z        for value in set(point[axis] for point in points_by_window[window_index]):
            for value in set(point[axis] for point in points_by_wall[wall_index]):
                group = group_points_by_axis(points_by_wall[wall_index], axis, value)
                sorted_group = sort_group(group)
                update_vertex_in_idf(ida_file_path, sorted_group)

# 调用主函数
ifc_file_path = r'path of ifc file'
ida_file_path = r'path of ida file'
Get_wall_vertices_world_coordinates(ifc_file_path,ida_file_path)