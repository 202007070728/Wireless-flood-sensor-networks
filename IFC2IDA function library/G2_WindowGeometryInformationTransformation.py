...
def Get_window_vertices_world_coordinates(ifc_file_path,ida_file_path):
    """
    ...
    for i, window in enumerate(windows):
        ...
        for j in range(0, len(verts), 3):
            ...
            window_vertices.append(np.round(transformed_vert[:3], 1).tolist())

        # Now wall_vertices contains all the vertices in world coordinates for this wall
        points_by_window[i + 1] = window_vertices

    # Print the points for each wall
    for wall_index, vertices in points_by_window.items():
        print(f"墙{wall_index}的顶点坐标：")
        # for vertex in vertices:
        # print(vertex)
        print(points_by_window[wall_index])  # Add an empty line for better readability between walls
        for axis in range(3):  # 0: x, 1: y, 2: z        for value in set(point[axis] for point in points_by_window[window_index]):
            for value in set(point[axis] for point in points_by_window[wall_index]):
                group = group_points_by_axis(points_by_window[wall_index], axis, value)
                sorted_group = sort_group(group)
...
Get_window_vertices_world_coordinates(ifc_file_path,ida_file_path)
