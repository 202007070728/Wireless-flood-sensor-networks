...
def update_vertex_in_idf(ida_file_path, coordinates):
    ...
    for line in lines:
        ...
        updated_lines.append(f'{door_index} 4 {coord_str} {rest}\n')
        print("已更新")
...
def Get_door_vertices_world_coordinates(ifc_file_path,ida_file_path):
    """
    Load an IFC file and extract world coordinates of all door vertices.

    :param ifc_file_path: Path to the IFC file
    :return: A list of tuples, where each tuple contains the door index and its vertices in world coordinates
    """
    # Load the IFC file
    ...
    for line in lines:
        ...
        if 'END_WALLS' in line:
            ...
            updated_lines.append(line)
            continue
    # Select all door entities
    doors = ifc_file.by_type('IfcDoor')

    # Prepare a list to hold all the vertices in world coordinates
    world_vertices = []
    points_by_door = {}
    # Iterate through each door to get its absolute coordinates
    for i, door in enumerate(doors):
        ...
        for j in range(0, len(verts), 3):
            ...
            door_vertices.append(np.round(transformed_vert[:3], 1).tolist())

        # Now door_vertices contains all the vertices in world coordinates for this door
        points_by_door[i + 1] = door_vertices

    # Print the points for each door
    for door_index, vertices in points_by_door.items():
        print(f"墙{door_index}的顶点坐标：")
        # for vertex in vertices:
        # print(vertex)
        print(points_by_door[door_index])  # Add an empty line for better readability between doors
        for axis in range(3):  # 0: x, 1: y, 2: z        for value in set(point[axis] for point in points_by_window[window_index]):
            for value in set(point[axis] for point in points_by_door[door_index]):
                group = group_points_by_axis(points_by_door[door_index], axis, value)
                sorted_group = sort_group(group)
...
Get_door_vertices_world_coordinates(ifc_file_path,ida_file_path)
