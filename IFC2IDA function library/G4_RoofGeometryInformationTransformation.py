...
def update_vertex_in_idf(ida_file_path, coordinates):
    ...
    for line in lines:
        ...
        updated_lines.append(f'{roof_index} 4 {coord_str} {rest}\n')
        print("已更新")
...
def Get_roof_vertices_world_coordinates(ifc_file_path,ida_file_path):
    """
    Load an IFC file and extract world coordinates of all roof vertices.

    :param ifc_file_path: Path to the IFC file
    :return: A list of tuples, where each tuple contains the roof index and its vertices in world coordinates
    """
    # Load the IFC file
    ...
    for i, roof in enumerate(roofs):
        ...
        for j in range(0, len(verts), 3):
            ...
            roof_vertices.append(np.round(transformed_vert[:3], 1).tolist())

        # Now roof_vertices contains all the vertices in world coordinates for this roof
        points_by_roof[i + 1] = roof_vertices

    # Print the points for each roof
    for roof_index, vertices in points_by_roof.items():
        print(f"墙{roof_index}的顶点坐标：")
        # for vertex in vertices:
        # print(vertex)
        print(points_by_roof[roof_index])  # Add an empty line for better readability between roofs
        for axis in range(3):  # 0: x, 1: y, 2: z        for value in set(point[axis] for point in points_by_window[window_index]):
            for value in set(point[axis] for point in points_by_roof[roof_index]):
                group = group_points_by_axis(points_by_roof[roof_index], axis, value)
                sorted_group = sort_group(group)
...
Get_roof_vertices_world_coordinates(ifc_file_path,ida_file_path)
